...
public class FabricConfigHelper implements IConfigHelper {
    ...
    public void ResetColor(String dimId) {
        switch (dimId) {
            case "minecraft:overworld" -> {
                ConfigFabric.get().OVERWORLD_COLOR = ConfigCommon.OVERWORLD_COLOR;
                ConfigFabric.saveConfig();
            }
            case "minecraft:the_nether" -> {
                ConfigFabric.get().NETHER_COLOR = ConfigCommon.NETHER_COLOR;
                ConfigFabric.saveConfig();
            }
            case "minecraft:the_end" -> {
                ConfigFabric.get().END_COLOR = ConfigCommon.END_COLOR;
                ConfigFabric.saveConfig();
            }
            default -> {
                ConfigFabric.get().MODDED_DIMS.removeIf(c -> dimId.equals(c.split(" ", 2)[0]));
                ConfigFabric.saveConfig();
            }
        }
    }
    ...
    public void SetPosition(String position) {
        try {
            ConfigFabric.get().DIM_POSITION = CommonUtils.DimensionPosition.valueOf(position.toUpperCase());
            ConfigFabric.saveConfig();
        } catch (IllegalArgumentException e) {
            Constants.LOG.error("Invalid position value: {}", position);
        }
    }

    @Override
    public void RemoveColor(String dimId) {
        ResetColor(dimId);
    }

    @Override
    public void RemoveAlias(String dimId) {
        ResetAlias(dimId);
    }
